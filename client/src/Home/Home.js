...
function Home(props) {
    ...
    const COLORS = ['#004953', '#006400', '#018749', '#17B169','#4FFFB0','#66FF00','#008080','#0000FF','#E000080'];
    const customStyles = {
        content: {
          top: '50%',
          left: '50%',
...
const renderCustomizedLabel = ({
    ...
    const handleChange=(event)=>{
      ...
      const handleChange_date1=(event)=>{
        formsetdate1(event.target.value);
        
      }
      const handleChange_date2=(event)=>{
        formsetdate2(event.target.value);
    ...
    <div className='main-container'>
        <div className='Nav_Bar'>
            ...
            MealFul AssignMent (By Aniket Katakdhond) 
            </div>
            <div>
      ...
      >
        ...
        <div>In the First Line Chart select the Date label on X axix to view its Time distribution in Line Chart 2</div>
        <div>For PieChart distribution:(Bonus Task)</div>
        <div>Select relevant dates</div>
        <div>In the legend, 0 means scheduled and item date are same, 1 means order scheduled 1 day before item_date</div>
        <div>And yes, click submit button to view results in both Graph1 and PieChart</div>
        ...
        <div className='date_form'>
            <form onSubmit={handleSubmit}>
                <label >
                    ...
                    <input type="text" value={itemdate} onChange={handleChange} className='date_input'/>
                    </label>
       ...
       <div style={{margin: 10}}>
       <Calendar className="calender-container_1"
            onChange={setDate} 
            value={date} 
       ...
       />
        ...
        <div className='linechart-container'>
            ...
            <ResponsiveContainer width="100%" aspect="3" >
                ...
                <LineChart data={pData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                    <CartesianGrid/>
                    <XAxis onClick={dateview} dataKey="date" 
        ...
        <div className='linechart-container'>
            <h1 className="text-heading">
                Time Distribution: {schdate}
            </h1>
            <ResponsiveContainer width="100%" aspect="3" >
                
                <BarChart data={tdata} margin={{ top: 5, right: 20, bottom: 5, left: 0 }} >
    ...
    <div  className='linechart-container'>
        ...
        <div className='row-container'>
            <form  style={{padding:70}} >
                <label>
                    Date:
                    <input type="text" value={formdate1} onChange={handleChange_date1} />
                    </label>
            ...
            <form onSubmit={handleSubmit_2dates} style={{padding:70}}>
                <label style={{padding:10}}>
                    Date:
                    <input type="text" value={formdate2} onChange={handleChange_date2} />
                    </label>
                    <input type="submit" value="Submit" />
            </form>
        </div>
